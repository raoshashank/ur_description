...
#define ball_target_radius = 190 //target ball radius in pixels
#define NF 3
#include <ur_modern_driver/ur_driver.h>
...
//#define CAMERA_TEST
#define VS_TEST1 0
object_tracking::coordinates current;
object_tracking::coordinates target;
sensor_msgs::JointState state;
// std::condition_variable rt_msg_cond_;
// std::condition_variable msg_cond_;
// std::string host = string("192.168.2.220");
...
public:
    RosWrapper(std::string host, int reverse_port) :
        as_(nh_, "follow_joint_trajectory",
            ...
            boost::bind(&RosWrapper::cancelCB, this, _1), false), robot_(rt_msg_cond_, msg_cond_, host, reverse_port, 0.03, 300), 
    ...
    void ravi_wants_to_use_this(std::string message) {
        //        std::cout<<"\n WOW Gollum we enters where to reach precious    :    "<<message<<std::endl;
        //        std::cout<<"\n WOW Gollum we enters where to reach precious    :    "<<robot_.rt_interface_->getLocalIp()<<std::endl;

        robot_.rt_interface_->addCommandToQueue(message);
    }
    void get_force(std::vector<double>& f)
    {
        f=robot_.rt_interface_->robot_state_->getTcpForce();

    }

    void stop_it()
    {
     robot_.rt_interface_->addCommandToQueue("stopj(10)\n");
...
private:
    ...
    void callback_images(const object_tracking::coordinates& c)
    {
        current = c;
    }
    void callback_joint(const sensor_msgs::JointState& state )
    {
        cout<<"~~~~~~~~~~~~~~~"<<endl;
        for(int i=0;i<6;i++){
          position[i] = state.position[i];
          velocity[i] = state.velocity[i];
        }
        cout<<position[0]<<endl<<velocity[0]<<endl; 
    ...
    int main(int argc,char** argv)
    {
        ...
        ros::NodeHandle nh;
        ros::Subscriber sub_images = nh.subscribe("this_topic",1000,callback_images);
        ros::Subscriber sub_joint_angles = nh.subscribe("joint_angles",1000,callback_joint);
       ...
       ros::waitForShutdown();
        ...
        //camera_model(x_t, u_t);  //get u_t from x_t
        ...
        for(double t = 0; t < TMAX; t = t+dt)
        {
            ...
            sprintf(cmd,"speedj([%1.5f, %1.5f, %1.5f, %1.5f, %1.5f, %1.5f], %f)\n",
                Theta_dot.at<double>(0,0),
                Theta_dot.at<double>(1,0),      
                Theta_dot.at<double>(2,0),
                Theta_dot.at<double>(3,0),
                Theta_dot.at<double>(4,0),
                Theta_dot.at<double>(5,0),
                1.0);
            
            cout<<"ADDING COMMAND TO QUEUE"<<endl;
            interface.ravi_wants_to_use_this(cmd);
